...
document.addEventListener("DOMContentLoaded", () => {
                            ...
                            setTimeout(
                                () => {
                                    ...
                                    loadingText.querySelectorAll(":scope > span[letter]")?.forEach((z, i) => {
                                        ...
                                        td = Math.max(
                                            td,
                                            getSpeed(getRoot("--Loading-Letter-Leave-Speed")) +
                                                i * getSpeed(getRoot("--Loading-Letter-Leave-Delay"))
                                        );
                                    });
    ...
    // Initialize Tippy.js for timeline skills tooltips
    tippy(".timeline-content .company", {
        content: (reference) => {
            const skills = reference.getAttribute("data-skills");
            return `<strong>Skills Used:</strong><br>  
            ${skills.split(", ").join("<br>")}`;
        },
        allowHTML: true,
        placement: "top",
        animation: "fade",
        theme: "light", // Use a light theme for better readability against dark backgrounds
        interactive: true
    });
    ...
    const cloud = document.querySelector(".language-cloud");

    if (cloud) {
        const positions = {
            HELLO: { x: 5, y: 8, size: 4 },
            నమస్కారం: { x: 45, y: 8, size: 4 },
            "GUTEN TAG": { x: 80, y: 8, size: 4 },
            BONJOUR: { x: 20, y: 20, size: 4 },
            OLÁ: { x: 65, y: 20, size: 4 },
            你好: { x: 82, y: 33, size: 5 },
            こんにちは: { x: 5, y: 42, size: 4 },
            مرحبا: { x: 82, y: 62, size: 4 },
            안녕하세요: { x: 5, y: 74, size: 4 },
            CIAO: { x: 20, y: 86, size: 4 },
            GOEDENDAG: { x: 62, y: 86, size: 4 },
            HOLA: { x: 5, y: 95, size: 4 },
            வணக்கம்: { x: 45, y: 95, size: 4 },
            नमस्ते: { x: 82, y: 95, size: 4 }
        };

        document.querySelectorAll(".language-cloud .lang").forEach((word) => {
            const text = word.textContent.trim();
            if (!positions[text]) return;
            const p = positions[text];
            word.style.left = `${p.x}%`;
            word.style.top = `${p.y}%`;
            //word.style.fontSize = `${p.size}rem`;
        });
